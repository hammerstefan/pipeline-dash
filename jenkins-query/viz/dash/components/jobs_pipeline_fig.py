...
def size_traces(scale: float,
    ...
    for et in edge_traces.values():
        et.line.width = max(et.line.width*scale, .5)
...
def resize_fig_data_from_scale(fig: dict, scale: float):
    ...
    for d in fig["data"]:  # type: dict
        ...
        if "line" in d:
            d["line"]["width"] = max(default_edge_width * scale, .5)
    for an in itertools.chain(fig["layout"].get("annotations", []),
                              fig["layout"]["updatemenus"][0]["buttons"][0]["args"][1]["annotations"]): # type: dict
...
def resize_fig_data_from_y_delta(fig: dict, new_y_delta: Optional[float]):
    ...
    scale = ((default_range[1] - default_range[0]) / new_y_delta * default_scaling) \
