...
class LeftPane(dbc.Col):
    ...
    def setup_expand_all_callbacks(cls, app: dash.Dash):
        ...
        @logged_callback
        def cb_expand_all(n_clicks: int, table_data, filtering) -> Any:
            if n_clicks is None:
                raise PreventUpdate()
        ...
        @logged_callback
        def cb_delayed_table_gen(n_intvl):
            if n_intvl is None:
                raise PreventUpdate()
    ...
    def setup_refresh_callbacks(
        ...
        @logged_callback
        def cb_enable_refresh(cb_value: bool, interval: int) -> tuple[bool, int]:
        ...
        @logged_callback
        def cb_refresh_now(n_clicks: int, *args, **kwargs) -> Any:
            if n_clicks is None:
                raise PreventUpdate()
    ...
    def setup_intvl_refresh_callback(
        ...
        @logged_callback
        def cb_intvl_refresh_trigger(nintervals, *args, **kwargs):
